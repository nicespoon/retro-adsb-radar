...
class DataTable:
    ...
    def draw(self, aircraft_list: List[Aircraft], status: str, last_update: float):
        ...
        # Column headers
        headers_y = self.rect.y + 30
        headers = ["CALL", "ALT", "SPD", "DIST", "HDG"]
        col_width = self.rect.width // 5

        for i, header in enumerate(headers):
            text = self.data_font.render(header, True, AMBER)
            self.screen.blit(text, (self.rect.x + 10 + i * col_width, headers_y))

        # Separator line
        pygame.draw.line(self.screen, DIM_GREEN,
                        (self.rect.x + 8, headers_y + 20),
                        (self.rect.right - 8, headers_y + 20), 2)
        ...
        for i, aircraft in enumerate(sorted_aircraft[:MAX_TABLE_ROWS]):  # Show up to MAX_TABLE_ROWS aircraft
            ...
            for j, value in enumerate(columns):
                ...
                self.screen.blit(text, (self.rect.x + 10 + j * col_width, y_pos))
        ...
        for i, info in enumerate(status_info):
            ...
            self.screen.blit(text, (self.rect.x + 10, status_y + i * 12))
...
def main():
    ...
    # Create components
    radar_size = min(SCREEN_HEIGHT - 120, SCREEN_WIDTH // 2 - 10) // 2
    radar = RadarScope(screen, SCREEN_WIDTH // 4, SCREEN_HEIGHT // 2 + 20, radar_size)

    table = DataTable(screen, SCREEN_WIDTH // 2, 60, 
                     SCREEN_WIDTH // 2 - 10, SCREEN_HEIGHT - 80)
    ...
    while running:
        ...
        screen.blit(instructions, (15, SCREEN_HEIGHT - 20))
